...
class FFTConv2D(Layer):
    ...
    def build(self, input_shape):
        self.kernel = self.add_weight(name='kernel',
                                      ...
                                      initializer='uniform',
                                      trainable=True)
        super(FFTConv2D, self).build(input_shape)

    def call(self, x):
        input_fft = tf.signal.rfft2d(x)
        weights_fft = tf.signal.rfft2d(self.kernel)
        input_fft_padded = tf.pad(input_fft, [[0, 0], [0, weights_fft.shape[0] - 1], [0, weights_fft.shape[1] - 1], [0, 0]])

        # Pad weights_fft to match the size of input_fft_padded
        pad_height = max(0, input_fft_padded.shape[1] - weights_fft.shape[0])
        pad_width = max(0, input_fft_padded.shape[2] - weights_fft.shape[1])
        pad_channels = max(0, input_fft_padded.shape[3] - weights_fft.shape[3] + 1)
        
        weights_fft_padded = tf.pad(weights_fft, [[0, pad_height],
                                                [0, pad_width],
                                                [0, 0],
                                                [0, pad_channels]])

        conv_fft = tf.multiply(input_fft_padded, weights_fft_padded)
        layer_output = tf.signal.irfft2d(conv_fft)
        if self.padding == 'same':
            layer_output = layer_output[:, :x.shape[1], :x.shape[2], :]
        return layer_output
...
#Instantiation
	...
	AlexNet.add(FFTConv2D(filters=96, input_shape=(32, 32, 3), kernel_size=(11, 11), strides=(4, 4), padding='same'))
	AlexNet.add(BatchNormalization())
	AlexNet.add(Activation('relu'))
	AlexNet.add(MaxPooling2D(pool_size=(2,2), strides=(2,2), padding='same'))

	#2nd Convolutional Layer
	AlexNet.add(FFTConv2D(filters=256, kernel_size=(5, 5), strides=(1, 1), padding='same'))
	AlexNet.add(BatchNormalization())
	AlexNet.add(Activation('relu'))
	AlexNet.add(MaxPooling2D(pool_size=(2,2), strides=(2,2), padding='same'))

	#3rd Convolutional Layer
	AlexNet.add(FFTConv2D(filters=384, kernel_size=(3, 3), strides=(1, 1), padding='same'))
	AlexNet.add(BatchNormalization())
	AlexNet.add(Activation('relu'))
	...
	AlexNet.add(FFTConv2D(filters=256, kernel_size=(3, 3), strides=(1, 1), padding='same'))
	AlexNet.add(BatchNormalization())
	AlexNet.add(Activation('relu'))
	AlexNet.add(MaxPooling2D(pool_size=(2,2), strides=(2,2), padding='same'))
